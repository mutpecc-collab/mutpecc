  ...
  const handleAssignCounselor = async (sessionId: string, counselorId: string) => {
    ...
    if (error) {
      ...
      toast({
        ...
        description: "Session has been assigned to the counselor.",
      });
      fetchData();
    }
  };
                          ...
                          <div className="flex items-center justify-between gap-4">
                            ...
                            <span className="text-xs text-muted-foreground">
                              ...
                              {booking.contacted_at &&
                                new Date(booking.contacted_at).toLocaleDateString()}
                            </span>
                          </div>
                        </div>
                      ))}
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              {pendingSessions.length > 0 && (
                <span className="ml-2 px-2 py-0.5 text-xs rounded-full bg-amber-100 text-amber-700">
                  {pendingSessions.length} unassigned
                </span>
              )}
            </CardTitle>
          </CardHeader>
          <CardContent>
            {loading ? (
              ...
              <div className="space-y-6">
                {/* Pending/Unassigned Sessions */}
                {pendingSessions.length > 0 && (
                  <div>
                    <h4 className="text-sm font-semibold text-foreground mb-3 flex items-center gap-2">
                      <Clock className="w-4 h-4 text-amber-500" />
                      Unassigned Sessions ({pendingSessions.length})
                    </h4>
                    <div className="space-y-3">
                        ...
                        >
                          <div className="flex items-start justify-between gap-4">
                            ...
                            <div className="flex flex-col gap-2 min-w-[180px]">
                              <Select
                                onValueChange={(value) =>
                                  handleAssignCounselor(session.id, value)
                                }
                              >
                                <SelectTrigger>
                                ...
                                <SelectContent>
                                  {counselors.map((counselor) => (
                                    ...
                                    >
                                      {counselor.full_name || counselor.email || "Unknown"}
                                    </SelectItem>
                                  ))}
                                </SelectContent>
                              </Select>
                ...
                {assignedSessions.length > 0 && (
                  <div>
                    <h4 className="text-sm font-semibold text-foreground mb-3 flex items-center gap-2">
                      <CheckCircle className="w-4 h-4 text-green-500" />
                      All Sessions ({assignedSessions.length})
                    </h4>
                    <div className="space-y-3">
